...
for year in range(2005,2015):
    ...
    tmpPred=pd.read_csv('mlp_multiclass_predict{}basedon{}.csv'.format(next_year,this_year),parse_dates=['date'])
...
for ii in dateVectorPnL[:-1]:
    ...
    if counter>4:
        counter=0
    longStocks=pd.DataFrame(proba.xs(ii,axis=0,level=1,drop_level=False)['2'].nlargest(stockNumber))
    shortStocks=pd.DataFrame(proba.xs(ii,axis=0,level=1,drop_level=False)['0'].nlargest(stockNumber))
...
portfolios.to_csv('mlpMulticlassReturnAnalyse.csv')